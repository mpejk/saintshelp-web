...
export async function POST(req: Request) {
    try {
        ...
        const { data: book, error: bErr } = await supabaseAdmin
            .from("books").select("id,storage_path,indexing_status").eq("id", bookId).single();
        if (bErr || !book) return Response.json({ error: "Book not found" }, { status: 404 });
        if (book.indexing_status !== "pending") {
            return Response.json({ error: `Book is already ${book.indexing_status}` }, { status: 400 });
        }
        ...
        return Response.json({ bookId, chunkCount: chunks.length, status: "ready" });
    } catch (err: any) {
